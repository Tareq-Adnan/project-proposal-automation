...
// $pdfFiles = ['cover.pdf','pre_pages.pdf', 'output.pdf','post_pages.pdf']; // Adjust the file names accordingly
    ...
    if($terms) {
        ...
        foreach ($nonEmptyRows as $key => $row) {
           // $data .= "<tr>";
            foreach ($nonEmptyColumns as $col) {
                $cellValue = $worksheet->getCellByColumnAndRow($col, $row)->getValue();
                if($key==1) $data.="<li><b>Payment Method</b>"."<ul><li>".$cellValue."</li></ul></li>";
